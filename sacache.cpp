...
unsigned long long extractBits(unsigned long long crux);

int main(int argc, char **argv) {
    ifstream infile(argv[1]);
    
    /* Ready the infile settings */
    ...
    for (int i = 0; i < 8; i++) 
    {
        for (int j = 0; j < 4096; j++) 
        {
            for (int k = 0; k < 10; k++) 
            {
                LRU[i][k] = -1; // init LRU checker
            }
            for (int m = 0; m < 3; m++) 
            {
                SACache[i][j][m] = -1; // init data storage
            }
        }
    }
    ...
    while (infile >> addr >> rw >> data ) 
    {
        ...
        if (rw == 0) { //read. call LRU 
            ...
            if (hit == 1) 
            { //found in positions 0 1 or 2 of LRU structure
                temp = LRU[setNum][foundPos]; //store found tag into temp
                
                if (foundPos == 0) 
                { //what we found is already at the top
                    
                }
                
                else if (foundPos == 1) 
                { //find(tag) = second. second = first, first = temp
                    LRU[setNum][1] = LRU[setNum][0];
                    LRU[setNum][0] = temp;
                }
                
                else if (foundPos == 2) 
                { //find(tag) = third. third = second, second = first, first = temp
                    LRU[setNum][2] = LRU[setNum][1]; //2 -> 3
                    LRU[setNum][1] = LRU[setNum][0]; //1 -> 2
                    LRU[setNum][0] = temp;
                }
                
                if (SACache[setNum][LRU[setNum][0]][offset] == -1) 
                { //if hit but not valid data
                    cout << left << hex << addr << left << setw(5) << hit << left << setw(5) << "0" << endl;
                }
                
                else if ( SACache[setNum][LRU[setNum][0]][offset] != -1) 
                { //hit and good data
                    cout << left << setw(3) << hex << SACache[setNum][LRU[setNum][0]][offset] << left << setw(2) << hit << left << setw(2) << SACache[setNum][LRU[setNum][0]][2] << endl;
                }
            }
        ...
        if (rw == 255) 
        { //write, call LRU then write to SACache[setNum][LRU[setNum][0]][offset]
            for (int i = 0; i < 3; i++) 
            {
                if (LRU[setNum][i] == tag) 
                {
            ...
            else if (hit == 0) 
            {
                ...
                SACache[setNum][LRU[setNum][0]][offset] = data;
                SACache[setNum][LRU[setNum][0]][2] = 1; //set dirty bit
